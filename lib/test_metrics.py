...
@torch.no_grad()
def eval_all_metrics(test_metrics: dict, x_real: torch.Tensor, x_fake: torch.Tensor, device=None):
    """
    x_real, x_fake: [B, S, D]
    returns:
      results: dict[name -> float]
      componentwise: dict[name -> torch.Tensor]  (per-feature or per-lag vector depending on metric)
      success: dict[name -> bool]
    """
    if device is None:
    ...
    for name, metric_ctor in test_metrics.items():
        metric = metric_ctor(x_real=x_real).to(device)   # instantiate with real stats
        val = metric(x_fake)                              # forward() returns scalar
        results[name] = float(val.item())
        componentwise[name] = metric.loss_componentwise.detach().cpu()
        success[name] = bool(metric.success.detach().cpu().item())

    return results, componentwise, success
