import os
import pickle

import numpy as np
import pandas as pd
import torch
from tqdm import tqdm


class Pipeline:
    def __init__(self, steps):
        """ Pre- and postprocessing pipeline. """
        self.steps = steps

    def transform(self, x, until=None):
        x = x.clone()
        for n, step in self.steps:
            if n == until:
                break
            x = step.transform(x)
        return x

    def inverse_transform(self, x, until=None):
        for n, step in self.steps[::-1]:
            if n == until:
                break
            x = step.inverse_transform(x)
        return x

"""
class StandardScalerTS():
    Standard scales a given (indexed) input vector along the specified axis. 

    def __init__(self, axis=(1)):
        self.mean = None
        self.std = None
        self.axis = axis

    def transform(self, x):
        if self.mean is None:
            self.mean = torch.mean(x, dim=self.axis)
            self.std = torch.std(x, dim=self.axis)
        return (x - self.mean.to(x.device)) / self.std.to(x.device)

    def inverse_transform(self, x):
        return x * self.std.to(x.device) + self.mean.to(x.device)
"""

class MinMaxScaler():
    """
    Min-Max scaler for time-series tensors (works for 3D and more).
    
    Typical 3D shape: (N, T, D) = (batch/blocks, time, features)
    
    axis:
      - axes over which to compute min/max (the axes you want to "normalize across")
      - Example for (N,T,D): axis=(0,1) scales each feature using global min/max over all samples+time
      - Example: axis=1 scales each (N,D) independently across time
    """

    def __init__(self, axis=(0, 1), feature_range=(0.05, 0.95), eps=1e-8, keepdim=True):
        self.axis = axis if isinstance(axis, (tuple, list)) else (axis,)
        self.feature_range = feature_range
        self.eps = eps
        self.keepdim = keepdim

        self.min = None
        self.max = None

    def _reduce_min(self, x):
        # torch.amin supports multi-dim reduction
        return torch.amin(x, dim=self.axis, keepdim=self.keepdim)

    def _reduce_max(self, x):
        return torch.amax(x, dim=self.axis, keepdim=self.keepdim)

    def fit(self, x):
        self.min = self._reduce_min(x).detach()
        self.max = self._reduce_max(x).detach()
        return self

    def transform(self, x):
        if self.min is None or self.max is None:
            self.fit(x)

        lo, hi = self.feature_range
        xmin = self.min.to(x.device)
        xmax = self.max.to(x.device)

        denom = (xmax - xmin).clamp_min(self.eps)
        x01 = (x - xmin) / denom                 # [0,1] (approximately)
        return x01 * (hi - lo) + lo              # [lo, hi]

    def inverse_transform(self, x):
        if self.min is None or self.max is None:
            raise RuntimeError("MinMaxScalerTS.inverse_transform() called before fit/transform.")

        lo, hi = self.feature_range
        xmin = self.min.to(x.device)
        xmax = self.max.to(x.device)

        denom = (hi - lo) if abs(hi - lo) > self.eps else self.eps
        x01 = (x - lo) / denom                   # back to [0,1]
        return x01 * (xmax - xmin) + xmin


def get_equities_dataset(assets=('SPX', 'DJI'), with_vol=True):
    """
    Get different returns series.
    """
    oxford = pd.read_csv('./data/oxfordmanrealizedvolatilityindices.csv')

    start = '2005-01-01 00:00:00+01:00'
    end = '2020-01-01 00:00:00+01:00'

    if assets == ('SPX',):
        df_asset = oxford[oxford['Symbol'] == '.SPX'].set_index(['Unnamed: 0'])  # [start:end]
        price = np.log(df_asset[['close_price']].values)
        rtn = (price[1:] - price[:-1]).reshape(1, -1, 1)
        vol = np.log(df_asset[['medrv']].values[-rtn.shape[1]:]).reshape(1, -1, 1)
        data_raw = np.concatenate([rtn, vol], axis=-1)
    elif assets == ('SPX', 'DJI'):
        df_spx = oxford[oxford['Symbol'] == '.SPX'].set_index(['Unnamed: 0'])[start:end]
        df_dji = oxford[oxford['Symbol'] == '.DJI'].set_index(['Unnamed: 0'])[start:end]
        index = df_dji.index.intersection(df_spx.index)
        df_dji = df_dji.loc[index]
        df_spx = df_spx.loc[index]
        price_spx = np.log(df_spx[['close_price']].values)
        rtn_spx = (price_spx[1:] - price_spx[:-1]).reshape(1, -1, 1)
        vol_spx = np.log(df_spx[['medrv']].values).reshape(1, -1, 1)
        price_dji = np.log(df_dji[['close_price']].values)
        rtn_dji = (price_dji[1:] - price_dji[:-1]).reshape(1, -1, 1)
        vol_dji = np.log(df_dji[['medrv']].values).reshape(1, -1, 1)
        data_raw = np.concatenate([rtn_spx, vol_spx[:, 1:], rtn_dji, vol_dji[:, 1:]], axis=-1)
    else:
        raise NotImplementedError()
    data_raw = torch.from_numpy(data_raw).float()
    
    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    #data_preprocessed = pipeline.transform(data_raw)
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_preprocessed = pipeline.transform(data_raw)
    
    print(data_preprocessed.shape)
    return pipeline, data_raw, data_preprocessed


def get_var_dataset(window_size, batch_size=5000, dim=3, phi=0.8, sigma=0.5):
    def multi_AR(window_size, dim=3, phi=0.8, sigma=0.5, burn_in=200):
        window_size = window_size + burn_in
        xt = np.zeros((window_size, dim))
        one = np.ones(dim)
        ide = np.identity(dim)
        MU = np.zeros(dim)
        COV = sigma * one + (1 - sigma) * ide
        W = np.random.multivariate_normal(MU, COV, window_size)
        for i in range(dim):
            xt[0, i] = 0
        for t in range(window_size - 1):
            xt[t + 1] = phi * xt[t] + W[t]
        return xt[burn_in:]

    var_samples = []
    for i in range(batch_size):
        tmp = multi_AR(window_size, dim, phi=phi, sigma=sigma)
        var_samples.append(tmp)
    data_raw = torch.from_numpy(np.array(var_samples)).float()

    def get_pipeline():
        transforms = list()
        #transforms.append(('standard_scale', StandardScalerTS(axis=(0, 1))))  # standard scale
        
        pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
        pipeline = Pipeline(steps=transforms)
        return pipeline

    pipeline = get_pipeline()
    data_preprocessed = pipeline.transform(data_raw)
    print(data_preprocessed.shape)
    return pipeline, data_raw, data_preprocessed


def get_arch_dataset(window_size, lag=4, bt=0.055, N=5000, dim=1):
    """
    Creates the dataset: loads data.

    :param data_path: :param t_lag: :param device: :return:
    """

    def get_raw_data(N=5000, lag=4, T=2000, omega=0.00001, bt=0.055, burn_in=2000):
        beta = bt * np.ones(lag)
        eps = np.random.randn(N, T + burn_in)
        logrtn = np.zeros((N, T + burn_in))

        initial_arch = omega / (1 - beta[0])

        arch = initial_arch + np.zeros((N, T + burn_in))

        logrtn[:, :lag] = np.sqrt(arch[:, :lag]) * eps[:, :lag]

        for t in range(lag - 1, T + burn_in - 1):
            arch[:, t + 1] = omega + np.matmul(beta.reshape(1, -1), np.square(
                logrtn[:, t - lag + 1:t + 1]).transpose())  # * (logrtn[:, t] < 0.)
            logrtn[:, t + 1] = np.sqrt(arch[:, t + 1]) * eps[:, t + 1]
        return arch[:, burn_in:], logrtn[:, burn_in:]

    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    _, logrtn = get_raw_data(T=window_size, N=N, bt=bt)
    data_raw = torch.from_numpy(logrtn[..., None]).float()
    data_pre = pipeline.transform(data_raw)
    return pipeline, data_raw, data_pre


def load_pickle(path):
    with open(path, 'rb') as f:
        return pickle.load(f)


def rolling_window(x, x_lag, add_batch_dim=True):
    if add_batch_dim:
        x = x[None, ...]
    return torch.cat([x[:, t:t + x_lag] for t in range(x.shape[1] - x_lag + 1)], dim=0)

def rolling_window_mcmc(input_x, x_lag):
    
    T, D = input_x.shape
    X = list()
    condition_time = list()

    time_range = np.linspace(0.05, 0.95, T).reshape(-1, 1)
    
    if isinstance(input_x, np.ndarray):
        input_x = torch.tensor(input_x, dtype=torch.float32)
        
    for ix in range(0, T - x_lag + 1):
        x_block = input_x[ix:ix + x_lag]
        time_stamp = torch.from_numpy(time_range[ix + 16 : ix + x_lag])
        X.append(x_block)
        condition_time.append(time_stamp)
        
    return torch.stack(X).float(), torch.stack(condition_time).float()


def get_mit_arrythmia_dataset(filenames):
    DATA_DIR = './data/mit-bih-arrhythmia-database-1.0.0/'
    import wfdb
    records = list()
    for fn in filenames:
        records.append(wfdb.rdsamp(os.path.join(DATA_DIR, fn), sampto=3000)[0][None, ...])
    records = np.concatenate(records, axis=0)
    records = np.log(5 * (records - records.min() + 1))
    data_raw = torch.from_numpy(records).float()
    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_pre = pipeline.transform(data_raw)
    return pipeline, data_raw, data_pre


def get_etth_dataset(dataset_name):
    path = './data'
    df = pd.read_csv(os.path.join(path, dataset_name), index_col=0)
    data_raw = torch.from_numpy(df.values[np.newaxis, ...]).float()

    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_pre = pipeline.transform(data_raw)

    return pipeline, data_raw, data_pre

def get_sine_dataset(length=40000, dim=3):
    data = list()
    for _ in range(dim):
        A = 1.5 # np.random.uniform(1, 3) 
        w = 0.1 #np.random.uniform(0, 0.1) 
        p = np.random.uniform(-3, 3)

        temp = [A * np.sin(w * i + p) for i in range(length)]
        data.append(np.asarray(temp).reshape(-1, 1))
    data = np.concatenate(data, axis=1)[np.newaxis, ...]
    data_raw = torch.torch.from_numpy(data).float()
    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_pre = pipeline.transform(data_raw)
    
    print(data_pre.shape)
    return pipeline, data_raw, data_pre
    
def get_square_dataset(length=40000, dim=3):
    data = list()
    for _ in range(dim):
        A = 1.5 # np.random.uniform(1, 3) 
        w = 0.1 #np.random.uniform(0, 0.1) 
        p = np.random.uniform(-3, 3)

        temp = [A * np.sign(np.sin(w * i + p)) for i in range(length)]
        data.append(np.asarray(temp).reshape(-1, 1))
    data = np.concatenate(data, axis=1)[np.newaxis, ...]
    data_raw = torch.torch.from_numpy(data).float()
    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_pre = pipeline.transform(data_raw)
    
    print(data_pre.shape)
    return pipeline, data_raw, data_pre

def get_triangle_dataset(length=40000, dim=3):
    data = list()
    for _ in range(dim):
        A = 1.5 # np.random.uniform(1, 3) 
        w = 0.1 #np.random.uniform(0, 0.1) 
        p = np.random.uniform(-3, 3)
        T = 2 * np.pi / w

        temp = [i + p for i in range(length)]
        for index, item in enumerate(temp):
            value = item % T
            if value < 0.25 * T:
                value = A * value * 2 / np.pi * w
            elif value < 0.75 * T:
                value = A * (2 - value * 2 / np.pi * w)
            else:
                value = A * (-4 + value * 2 / np.pi * w)
            temp[index] = value
        data.append(np.asarray(temp).reshape(-1, 1))
    data = np.concatenate(data, axis=1)[np.newaxis, ...]
    data_raw = torch.torch.from_numpy(data).float()
    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_pre = pipeline.transform(data_raw)
    
    print(data_pre.shape)
    return pipeline, data_raw, data_pre

def get_sawtooth_dataset(length=40000, dim=3):
    data = list()
    for _ in range(dim):
        A = 1.5 # np.random.uniform(1, 3) 
        w = 0.1 #np.random.uniform(0, 0.1) 
        p = np.random.uniform(-3, 3)
        T = 2 * np.pi / w

        temp = [i + p for i in range(length)]
        for index, item in enumerate(temp):
            value = item % T
            value = A * (-1 + value / np.pi * w)
            temp[index] = value
        data.append(np.asarray(temp).reshape(-1, 1))
    data = np.concatenate(data, axis=1)[np.newaxis, ...]
    data_raw = torch.torch.from_numpy(data).float()
    #pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    pipeline = Pipeline(steps=[('minmax_scale', MinMaxScaler(feature_range=(0.05, 0.95)))])
    data_pre = pipeline.transform(data_raw)
    
    print(data_pre.shape)
    return pipeline, data_raw, data_pre

def get_data(data_type, p, q, **data_params):
    if data_type == 'VAR':
        pipeline, x_real_raw, x_real = get_var_dataset(
            40000, batch_size=1, **data_params
        )
    elif data_type == 'STOCKS':
        pipeline, x_real_raw, x_real = get_equities_dataset(**data_params)
    elif data_type == 'ARCH':
        pipeline, x_real_raw, x_real = get_arch_dataset(
            40000, N=1, **data_params
        )
    elif data_type == 'ECG':
        pipeline, x_real_raw, x_real = get_mit_arrythmia_dataset(**data_params)
    elif data_type == 'sine':
        pipeline, x_real_raw, x_real = get_sine_dataset(**data_params)
    elif data_type == 'square':
        pipeline, x_real_raw, x_real = get_square_dataset(**data_params)
    elif data_type == 'triangle':
        pipeline, x_real_raw, x_real = get_triangle_dataset(**data_params)
    elif data_type == 'sawtooth':
        pipeline, x_real_raw, x_real = get_sawtooth_dataset(**data_params)
    elif data_type == 'etth1':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTh1.csv')
    elif data_type == 'etth2':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTh2.csv')
    elif data_type == 'ettm1':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTm1.csv')
    elif data_type == 'ettm2':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTm2.csv')
    elif data_type == 'licor':
        pipeline, x_real_raw, x_real = get_etth_dataset('licor.csv')
    elif data_type == 'lorenz':
        pipeline, x_real_raw, x_real = get_etth_dataset('lorenz.csv')
    elif data_type == 'ILI':
        pipeline, x_real_raw, x_real = get_etth_dataset('ILI.csv')
    elif data_type == 'us_births':
        pipeline, x_real_raw, x_real = get_etth_dataset('us_births.csv')
    else:
        raise NotImplementedError('Dataset %s not valid' % data_type)
    assert x_real.shape[0] == 1
    x_real_block = rolling_window(x_real[0], p + q)
    return pipeline, x_real, x_real_block

def get_data_mcmc(data_type, p, q, **data_params):
    if data_type == 'VAR':
        pipeline, x_real_raw, x_real = get_var_dataset(
            40000, batch_size=1, **data_params
        )
    elif data_type == 'STOCKS':
        pipeline, x_real_raw, x_real = get_equities_dataset(**data_params)
    elif data_type == 'ARCH':
        pipeline, x_real_raw, x_real = get_arch_dataset(
            40000, N=1, **data_params
        )
    elif data_type == 'ECG':
        pipeline, x_real_raw, x_real = get_mit_arrythmia_dataset(**data_params)
    elif data_type == 'sine':
        pipeline, x_real_raw, x_real = get_sine_dataset(**data_params)
    elif data_type == 'square':
        pipeline, x_real_raw, x_real = get_square_dataset(**data_params)
    elif data_type == 'triangle':
        pipeline, x_real_raw, x_real = get_triangle_dataset(**data_params)
    elif data_type == 'sawtooth':
        pipeline, x_real_raw, x_real = get_sawtooth_dataset(**data_params)
    elif data_type == 'etth1':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTh1.csv')
    elif data_type == 'etth2':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTh2.csv')
    elif data_type == 'ettm1':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTm1.csv')
    elif data_type == 'ettm2':
        pipeline, x_real_raw, x_real = get_etth_dataset('ETTm2.csv')
    elif data_type == 'licor':
        pipeline, x_real_raw, x_real = get_etth_dataset('licor.csv')
    elif data_type == 'lorenz':
        pipeline, x_real_raw, x_real = get_etth_dataset('lorenz.csv')
    elif data_type == 'ILI':
        pipeline, x_real_raw, x_real = get_etth_dataset('ILI.csv')
    elif data_type == 'us_births':
        pipeline, x_real_raw, x_real = get_etth_dataset('us_births.csv')
    else:
        raise NotImplementedError('Dataset %s not valid' % data_type)
    assert x_real.shape[0] == 1
    x_real_block, condition_block = rolling_window_mcmc(x_real[0], p + q)
    return pipeline, x_real, x_real_block, condition_block


def download_man_ahl_dataset():
    import requests
    from zipfile import ZipFile
    url = 'https://realized.oxford-man.ox.ac.uk/images/oxfordmanrealizedvolatilityindices.zip'
    r = requests.get(url)
    with open('./oxford.zip', 'wb') as f:
        pbar = tqdm(unit="B", total=int(r.headers['Content-Length']))
        for chunk in r.iter_content(chunk_size=100 * 1024):
            if chunk:
                pbar.update(len(chunk))
                f.write(r.content)
    zf = ZipFile('./oxford.zip')
    zf.extractall(path='./data')
    zf.close()
    os.remove('./oxford.zip')


def download_mit_ecg_dataset():
    import requests
    from zipfile import ZipFile
    url = 'https://storage.googleapis.com/mitdb-1.0.0.physionet.org/mit-bih-arrhythmia-database-1.0.0.zip'
    r = requests.get(url)
    with open('./mit_db.zip', 'wb') as f:
        pbar = tqdm(unit="B", total=int(r.headers['Content-Length']))
        for chunk in r.iter_content(chunk_size=100 * 1024):
            if chunk:
                pbar.update(len(chunk))
                f.write(r.content)
    zf = ZipFile('./mit_db.zip')
    zf.extractall(path='./data')
    zf.close()
    os.remove('./mit_db.zip')

if __name__ == '__main__':
    get_equities_dataset()
    get_var_dataset(window_size=40000, batch_size=1)
    get_sine_dataset()
    get_square_dataset()
    get_triangle_dataset()
    get_sawtooth_dataset()
