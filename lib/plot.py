...
def plot_summary(x_fake, x_real, max_lag=None, labels=None):
    ...
    dim = min(x_real.shape[2], 8) # 最多画几个dim
...
def create_summary(dataset, device, G, lags_past, steps, x_real, one=False, experiment_directory=None):
    ...
    with torch.no_grad():
        ...
        if len(x_fake_future) == 2:
            if experiment_directory is not None:
                plt.plot(x_fake_future[1].cpu().numpy()[0, :2000])            
                savefig('long_path_ec.png', experiment_directory)
            x_fake_future = x_fake_future[0]
        if len(x_fake_future) == 3:
            if experiment_directory is not None:
                plt.plot(x_fake_future[1].cpu().numpy()[0, :2000])            
                plt.plot(x_fake_future[2].cpu().numpy()[0, :2000])            
                savefig('long_path_ec.png', experiment_directory)
            x_fake_future = x_fake_future[0]
