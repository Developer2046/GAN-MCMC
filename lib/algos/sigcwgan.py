...
def calibrate_sigw1_metric(config, x_future, x_past):
    ...
    lm = LinearRegression()
    lm.fit(X, Y)
    sigs_pred = torch.from_numpy(lm.predict(X)).float().to(x_future.device) # 这里做linear regression是因为要获得的是condition分布
    return sigs_pred
...
class SigCWGAN(BaseAlgo):
    ...
    def sample_batch(self, ):
        ...
        random_indices = random_indices.detach().cpu() if torch.is_tensor(random_indices) else random_indices
