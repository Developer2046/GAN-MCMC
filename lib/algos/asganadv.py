...
class ECG(nn.Module):
    ...
    def sample(self, steps, x_past):
        ...
        # the shape of the noise is : [batch_size, steps, dimension of the time series]
        z = torch.randn(x_past.size(0), steps, self.latent_dim).to(x_past.device)
        #print("Input Noise Dimension for the AECGAN : ", z.shape)
        if steps > self.q:
            # train_step
...
class AECGAN(BaseAlgo, ):
    def __init__(self, base_config, x_real):
        ...
        self.D_optimizer = torch.optim.Adam(self.D.parameters(), lr=2e-4, betas=(0.0, 0.9))  # Using TTUR
        self.G_optimizer = torch.optim.Adam([
            {'params': self.G.G.parameters(), 'lr': 1e-4, 'betas':(0.0, 0.9)},
    ...
    def step(self):
        ...
        for i in range(self.D_steps_per_G_step):
            # generate x_fake
            indices = sample_indices(self.x_real.shape[0], self.batch_size).to(self.device)
            
            indices = indices.detach().cpu() if torch.is_tensor(indices) else indices
